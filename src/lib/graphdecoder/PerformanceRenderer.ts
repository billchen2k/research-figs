...
export default class PerformanceRenderer extends SVGBox {
  ...
  public async draw() {
    ...
    const perSimData = await d3.csv('/data/graphdecoder/perSim.csv');
    const struSimData = await d3.csv('/data/graphdecoder/struSim.csv');
    const timePerfData = await d3.csv('/data/graphdecoder/timePerf.csv');
